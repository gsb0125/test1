	...
	if client.get_channel(channel) != msg.channel :
		...
		if message.content.startswith('!리젠'):
			embed = discord.Embed(
					title='----- 리스폰 보스 -----',
					description= ' ')
			embed.add_field(name='3시간', value='렌트로프초원', inline=False)
			embed.add_field(name='4시간', value='잊혀진사막', inline=False)
			embed.add_field(name='5시간', value='악이물든곳', inline=False)
			embed.add_field(name='6시간', value='나르바쉬,라데스성흔', inline=False)
			embed.add_field(name='7시간', value='북녘땅', inline=False)
			embed.add_field(name='8시간', value='루카던', inline=False)
			embed.add_field(name='9시간', value='타르타로스', inline=False)
			embed.add_field(name='12시간', value='로자미어', inline=False)
			await client.get_channel(channel).send(embed=embed, tts=False)
